...
def main():
    log.info("=" * 48)
    log.info("9router 代理池同步启动（主源 proxy-socks5 + GitHub databay-labs 备用）")
    ...
    # 备用源: GitHub databay-labs（仅当主源失败/为空时启用）
    if len(all_proxies) == 0:
        log.warning("⚠️ proxy-socks5.com 无代理，启用 GitHub databay-labs 备用源...")
